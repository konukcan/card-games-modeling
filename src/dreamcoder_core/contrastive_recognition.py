...
class ContrastiveRecognitionModel(nn.Module):
    ...
    def __init__(
        ...
        # Output heads
        self.primitive_head = PrimitiveHead(card_out, pred_hidden, self.num_primitives)
        self.bigram_head = BigramHead(card_out, pred_hidden, max_bigrams)
        self.count_head = CountHead(card_out)
    ...
    def encode_task(self, task) -> torch.Tensor:
        ...
        return self.task_encoder(pos_hands, neg_hands)
    ...
    def encode_task_batched(self, task) -> torch.Tensor:
        ...
        # Batch encode positive examples
        if pos_hands_data:
            suits = torch.stack([h[0] for h in pos_hands_data]).to(self.device)
            ranks = torch.stack([h[1] for h in pos_hands_data]).to(self.device)
            positions = torch.stack([h[2] for h in pos_hands_data]).to(self.device)
            masks = torch.stack([h[3] for h in pos_hands_data]).to(self.device)

            pos_embeddings = self.hand_encoder(suits, ranks, positions, masks)
            pos_mean = pos_embeddings.mean(dim=0)
        else:
            pos_mean = torch.zeros(hidden_dim, device=self.device)

        # Batch encode negative examples
        if neg_hands_data:
            suits = torch.stack([h[0] for h in neg_hands_data]).to(self.device)
            ranks = torch.stack([h[1] for h in neg_hands_data]).to(self.device)
            positions = torch.stack([h[2] for h in neg_hands_data]).to(self.device)
            masks = torch.stack([h[3] for h in neg_hands_data]).to(self.device)

            neg_embeddings = self.hand_encoder(suits, ranks, positions, masks)
            neg_mean = neg_embeddings.mean(dim=0)
        else:
            neg_mean = torch.zeros(hidden_dim, device=self.device)
    ...
    def predict_grammar_weights(self, task) -> Grammar:
        ...
        Blends original grammar weights with predicted probabilities.
        """
        probs = self.predict_primitives(task).cpu().numpy()
        ...
        for i, prod in enumerate(self.grammar.productions):
            ...
            if prim_name in self.primitive_to_idx:
                ...
                # Convert probability to log-probability and blend
                pred_log = math.log(max(probs[idx], 1e-10))
                new_lp = 0.5 * prod.log_probability + 0.5 * pred_log
            else:
