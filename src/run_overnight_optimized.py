#!/usr/bin/env python3
"""
Optimized Overnight Pre-training Runner

This script runs optimized staged curriculum pre-training with:
1. Early pruning in all-or-nothing mode (1.5-2x speedup)
2. Multiprocessing with PyPy workers (additional 3-6x speedup)

Combined expected speedup: 5-12x faster than original
- Original Phase 3: ~2 hours per iteration
- Optimized Phase 3: ~10-20 minutes per iteration

Architecture:
- Main process (CPython): DreamCoder loop, neural training, compression
- Worker subprocesses (PyPy): Parallel enumeration for tasks
...
# ============================================================================
# OPTIMIZED STAGED DREAMCODER
# ============================================================================

class OptimizedStagedDreamCoder:
    """
    DreamCoder with staged curriculum and parallel enumeration.

    Key optimizations:
    1. Early pruning in all-or-nothing mode
    2. Parallel task enumeration (optionally with PyPy workers)
    """
    ...
    def run(self) -> Dict:
        ...
        print_banner("DREAMCODER V2 - OPTIMIZED OVERNIGHT PRETRAINING")
        ...
        print(f"  - PyPy acceleration: {'YES' if self.use_pypy else 'NO (not available)'}")
        print()
        print("Phases:")
        total_iters = 0
            ...
            if isinstance(result, tuple):
                ...
                if result.get('solved'):
                    ...
                    for sol in result.get('solutions', []):
                        # We'd need to reconstruct Program from string here
                        # For now, just mark as solved
                        tm = self.task_metrics[task.name]
                        if not tm.solved:
                            tm.solved = True
                            tm.iteration_solved = self.global_iteration
                            tm.programs_to_solve = sol['programs_enumerated']
                            tm.best_program = sol['program']
                            self.log(f"  SOLVED: {task.name} ({sol['programs_enumerated']:,} programs)", 1)
    ...
    def _enumerate_parallel(self, tasks: List[Task], budget: int, max_depth: int) -> List[Dict]:
        ...
        if self.use_pypy and self.worker_script:
            ...
            # Fallback to CPython multiprocessing with early pruning
            self.log(f"  Using CPython parallel enumeration...")
            for task in tasks:
    ...
    def _save_checkpoint(self, name: str):
        ...
        # Save grammar - use dill if available (handles lambdas), else skip
        ...
        # Save frontiers - programs may also have lambdas
        frontiers_path = self.log_dir / f"frontiers_{name}_{timestamp}.pkl"
        try:
            import dill
            with open(frontiers_path, 'wb') as f:
                dill.dump(self.frontiers, f)
        except ImportError:
            ...
            with open(frontiers_path.with_suffix('.json'), 'w') as f:
                json.dump(frontiers_summary, f, indent=2)
        except Exception as e:
            self.log(f"Warning: Could not save frontiers: {e}")
    ...
    def _save_results(self, results: Dict):
        ...
        json_path = self.log_dir / f"overnight_optimized_{timestamp}.json"
    ...
    def _generate_report(self, results: Dict, timestamp: str):
        ...
        with open(report_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("DREAMCODER V2 - OPTIMIZED OVERNIGHT PRETRAINING REPORT\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write(f"PyPy used: {results['config']['optimizations']['pypy_used']}\n\n")
...
def main():
    ...
    log_dir = Path("results/overnight_optimized")
    ...
    print_banner("STARTING OPTIMIZED STAGED OVERNIGHT TRAINING")

    dc = OptimizedStagedDreamCoder(
        grammar=grammar,
    ...
    print_banner("OPTIMIZED OVERNIGHT PRETRAINING COMPLETE")
    print(f"End time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Total time: {format_time(total_time)}")
    print()
