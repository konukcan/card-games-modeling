#!/usr/bin/env python3
"""
Overnight Pre-training Runner with Set Transformer Recognition Model

This script runs staged curriculum pre-training with:
1. New Set Transformer recognition model architecture
2. Early pruning in all-or-nothing mode
3. Multiprocessing with PyPy workers for parallel enumeration
4. New halves primitives (first_half, second_half)

Key improvements over legacy GRU-based recognition:
- Hierarchical set encoding: cards -> hand -> task
- Position-aware attention for positional rules
- Learnable pooling via PMA (Pooling by Multihead Attention)
- Better task discrimination through self-attention

See docs/recognition_model_analysis_2024_12_03.md for details.
"""
...
# Primitives (now includes first_half, second_half)
...
def enumerate_task(task_data, grammar_productions, max_depth, max_programs, timeout):
    ...
    # Build grammar (now includes first_half, second_half primitives)
...
def create_worker_script():
    ...
    worker_path = Path(__file__).parent / 'enumeration_worker_set_transformer.py'
...
# ============================================================================
# SEQUENTIAL FALLBACK WITH EARLY PRUNING
# ============================================================================
...
# ============================================================================
# SET TRANSFORMER DREAMCODER
# ============================================================================

class SetTransformerDreamCoder:
    """
    DreamCoder with Set Transformer recognition model.

    Key features:
    1. Set Transformer for hierarchical set encoding
    2. Position-aware attention for positional rules
    3. Early pruning in all-or-nothing mode
    4. Parallel task enumeration (with PyPy workers)
    """
    ...
    def __init__(
        ...
        # Initialize SET TRANSFORMER recognition model (key change!)
        self.recognition = SetTransformerRecognitionModel(
            grammar=grammar,
            d_model=d_model,
            num_heads=num_heads,
            num_hand_layers=num_hand_layers,
            num_task_layers=num_task_layers,
            max_examples=20,
            max_cards=8,
            use_positional=True,  # Enable positional encoding for position-sensitive rules
            learning_rate=learning_rate,
            dropout=0.1,
            device=device
        )

        # Initialize dreamer (uses recognition model)
    ...
    def run(self) -> Dict:
        ...
        print_banner("DREAMCODER V2 - SET TRANSFORMER RECOGNITION MODEL")
        print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Easy tasks: {len(self.easy_tasks)}")
        print(f"All tasks: {len(self.all_tasks)}")
        print()
        print("SET TRANSFORMER CONFIG:")
        print(f"  - d_model: {self.recognition.d_model}")
        print(f"  - num_heads: 4")
        print(f"  - hand_layers: 2")
        print(f"  - task_layers: 2")
        print(f"  - positional_encoding: ENABLED")
        print(f"  - max_examples: 20")
        print(f"  - max_cards: 8")
        total_params = sum(p.numel() for p in self.recognition.parameters())
        ...
        for i, phase in enumerate(self.phases):
            tasks_str = "all" if phase.use_all_rules else "easy"
        ...
        print_banner("SET TRANSFORMER OVERNIGHT PRETRAINING COMPLETE")
    ...
    def _run_iteration(
        ...
        # =====================
        # SLEEP - RECOGNITION (Set Transformer)
        # =====================
        recognition_loss = 0.0
        recognition_time = 0.0

        self.log("\n[SLEEP - RECOGNITION] Training Set Transformer model...")
        rec_start = time.time()
    ...
    def _enumerate_parallel(self, tasks: List[Task], budget: int, max_depth: int) -> List[Dict]:
        ...
        if self.use_pypy and self.worker_script:
            ...
            self.log(f"  Using sequential enumeration...")
            results = []
            for task in tasks:
    ...
    def _enumerate_sequential(self, tasks: List[Task], budget: int, max_depth: int) -> List[Tuple]:
        """Enumerate tasks sequentially with early pruning."""
        results = []
        for task in tasks:
            frontier, programs = enumerate_task_with_early_pruning(
                self.grammar, task, self.eval_fn,
    ...
    def _save_iteration_checkpoint(self, iteration: int, metrics: 'IterationMetrics'):
        ...
        # Compute Set Transformer-specific metrics for interpretability
        try:
            task_discrimination = self.recognition.compute_task_discrimination(
                self.all_tasks[:30], n_sample=30
            )
        except Exception as e:
            task_discrimination = {'error': str(e)}
            self.log(f"Warning: Task discrimination computation failed - {e}")

        try:
            prediction_diversity = self.recognition.get_prediction_diversity(
                self.all_tasks[:30], n_sample=30
            )
        except Exception as e:
            prediction_diversity = {'error': str(e)}
            self.log(f"Warning: Prediction diversity computation failed - {e}")
        ...
        checkpoint = {
            ...
            # Set Transformer-specific metrics
    ...
    def _compile_results(self, total_time: float) -> Dict:
        ...
        return {
            'config': {
                ...
                'set_transformer': {
                    'd_model': self.recognition.d_model,
                    'num_heads': 4,
                    'hand_layers': 2,
                    'task_layers': 2,
            ...
            'recognition_training_losses': getattr(self.recognition, 'training_losses', [])
        }
    ...
    def _save_results(self, results: Dict):
        ...
        json_path = self.log_dir / f"overnight_set_transformer_{timestamp}.json"
        with open(json_path, 'w') as f:
    ...
    def _generate_report(self, results: Dict, timestamp: str):
        ...
        report_path = self.log_dir / f"overnight_set_transformer_report_{timestamp}.txt"

        with open(report_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("DREAMCODER V2 - SET TRANSFORMER RECOGNITION MODEL REPORT\n")
            f.write("=" * 80 + "\n\n")

            f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Duration: {format_time(results['summary']['total_time'])}\n\n")

            f.write("SET TRANSFORMER CONFIGURATION\n")
            f.write("-" * 40 + "\n")
            st_config = results['config']['set_transformer']
            f.write(f"d_model: {st_config['d_model']}\n")
            f.write(f"num_heads: {st_config['num_heads']}\n")
            f.write(f"hand_layers: {st_config['hand_layers']}\n")
            f.write(f"task_layers: {st_config['task_layers']}\n")
            f.write(f"positional_encoding: {st_config['positional_encoding']}\n")
            f.write(f"max_examples: {st_config['max_examples']}\n")
            f.write(f"max_cards: {st_config['max_cards']}\n\n")
...
def main():
    ...
    print_banner("DREAMCODER V2 - SET TRANSFORMER OVERNIGHT PRETRAINING")
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    ...
    # Build grammar (now includes first_half, second_half primitives)
    print("\nBuilding lean grammar (with halves primitives)...")
    grammar = build_lean_grammar()
    print(f"Grammar: {len(grammar)} primitives")

    # Verify halves primitives are present
    prim_names = [str(p.program) for p in grammar.productions]
    if 'first_half' in prim_names and 'second_half' in prim_names:
        print("  - first_half: present")
        print("  - second_half: present")
    else:
        print("  WARNING: halves primitives not found!")
    ...
    log_dir = Path("results/overnight_set_transformer")
    log_dir.mkdir(parents=True, exist_ok=True)

    # Create eval function
    eval_fn = make_eval_fn()

    # Run Set Transformer training
    print_banner("STARTING SET TRANSFORMER STAGED OVERNIGHT TRAINING")

    dc = SetTransformerDreamCoder(
        grammar=grammar,
