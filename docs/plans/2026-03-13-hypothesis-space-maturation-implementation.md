# Hypothesis Space Maturation Implementation Plan
...
**3c. In `score_rule()`, add `scoring_grammar` parameter:**

The `score_rule()` function reads `cls["canonical_prior"]` and `cls["summed_prior"]` — these are set during `build_hypothesis_pool()` using the *enumeration* grammar. When `scoring_grammar="weighted"`, we need to recompute priors.
...
```python
from gallery_analysis.dsl_prior import compute_log_prior as _compute_log_prior

# ... inside the loop over equivalence_classes ...

        # Select prior
        if grammar is not None:
            # Recompute prior under the provided (weighted) grammar
            log_prior_recomputed = _recompute_class_prior(cls, grammar)
            log_prior = log_prior_recomputed
...
In the `run_analysis()` scoring loop, pass the grammar:
...
def _is_shallow_top1(program_str: str) -> bool:
    ...
    # Programs like "(λ has_suit $0 SPADES)" or "has_suit"
...
def main():
    ...
    # Build grammars
    g_uniform = build_gallery_grammar()
...
### Task 6: Identify and translate additional LLM foils (Phase 2)

**Files:**
- Modify: `src/gallery_analysis/data/injected_hypotheses.json`

**Context:**
This task involves identifying untranslated LLM-generated hypotheses from the adjacent card-games project (or from `llm_hypotheses_raw.json`) and translating them into DSL format. The exact set of hypotheses depends on what's available at execution time.

**Step 1: Audit existing hypotheses**

Run: `cd src && python3 -c "
import json
with open('gallery_analysis/data/injected_hypotheses.json') as f:
    data = json.load(f)
print(f'Current injected: {len(data)}')
with open('gallery_analysis/data/llm_hypotheses_raw.json') as f:
    raw = json.load(f)
print(f'Raw hypotheses: {len(raw)}')
# Find which raw hypotheses don't have translations yet
translated_ids = {h.get('id', '') for h in data}
untranslated = [h for h in raw if h.get('id', '') not in translated_ids]
print(f'Untranslated: {len(untranslated)}')
for h in untranslated[:5]:
    print(f'  {h.get(\"id\", \"?\")}: {h.get(\"hypothesis_text\", \"?\")[:80]}')
"
```

Expected: A count of untranslated hypotheses.

**Step 2: Translate and add hypotheses**

For each untranslated hypothesis:
1. Read the `hypothesis_text` and `python_lambda` fields
2. Manually translate the Python lambda into a DSL program string
3. Add an entry to `injected_hypotheses.json` following the established format:
```json
{
    "id": "llm__{rule_id}__{hypN}",
    "source": "llm_foil",
    "true_for_rule": null,
    "dsl_program": "(λ ...)",
    "origin": {
...
**Step 3: Validate all injections parse correctly**

Run: `cd src && python3 -c "
from gallery_analysis.injection import load_and_validate_injections
from gallery_analysis.enumerator import build_weighted_gallery_grammar
g = build_weighted_gallery_grammar()
inj = load_and_validate_injections('gallery_analysis/data/injected_hypotheses.json', grammar=g)
print(f'Successfully validated {len(inj)} hypotheses')
"
`
Expected: All hypotheses validate. No parse errors.

**Step 4: Commit**

```bash
git add src/gallery_analysis/data/injected_hypotheses.json
git commit -m "feat: expand LLM foil injection set

Adds N additional LLM-generated confuser hypotheses translated from
raw Gemini Flash outputs. Total injection set now M hypotheses."
```

---

### Task 7: Full validation run with weighted grammar

**Files:**
- No new files — this is a validation run
...
**Step 2: Run the full analysis pipeline with weighted grammar**
...
Expected: Complete run with results saved to JSON.

**Step 3: Verify success criteria**

Check the output against design success criteria:
1. `has_suit`/`has_color` as top-1 ≤ 2/60 rules
2. Compositional programs visible in top-10 for rules like `three_or_more_same_suit`
3. Well-recovered rules (`all_same_suit`, `three_spades`) remain sharp
4. Results JSON saved correctly
...
### Task 8: Run existing tests to confirm no regressions

**Files:**
- No changes — regression check only

**Step 1: Run all existing tests**

Run: `cd src && python -m pytest tests/ -v --timeout=120 2>&1 | tail -30`
Expected: All existing tests PASS (the new weighted grammar doesn't change any default behavior — `build_gallery_grammar()` still returns uniform).

**Step 2: If any tests fail, fix them**

The weighted grammar changes should be backward-compatible since:
- `build_gallery_grammar()` is unchanged (still uniform)
- `score_rule()` defaults to `grammar=None` (uses stored priors)
- `run_analysis()` defaults to `scoring_grammar="uniform"`
- All new behavior is opt-in via flags

**Step 3: Commit if any fixes were needed**

```bash
git add -A
git commit -m "fix: resolve test regressions from weighted grammar integration"
```
