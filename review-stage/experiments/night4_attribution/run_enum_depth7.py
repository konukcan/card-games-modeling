"""
Night 4 — Enumeration runner (depth-7, 500k programs, uniform grammar, NO injections).

C2 arm of the attribution experiment. Builds ONE equivalence-class pool shared
across all 6 rules. Tests the budget-cap hypothesis from RESIDUAL_FINDING.md:
if the d6/300k enum pool was truncated before reaching the specific variants
MCMC visits, then d7/500k should either close the gap (budget was the issue)
or leave it roughly where it was (MCMC under-convergence or a different cause).

Deltas from night3/run_enum_depth6.py:
  - depth 6 → 7, max_programs 300k → 500k (both from night4/config.json).
  - Rule list reads from CONFIG["rules"] (flat 6-rule list for night4) instead
    of CONFIG["rules_night2"] + CONFIG["rules_new"] (night3 shape).
  - Output dir from CONFIG["enumeration"]["output_dir"] (enum_depth7_500k).
  - Fingerprint probe set preserved (500 probes, seed=42, uniform grammar) so
    classes are directly comparable to night3/enum_depth6_300k by fingerprint.

Everything else — adaptive extension ladder, per-rule posteriors, JSON sidecars,
--skip-if-exists semantics — is identical to the night3 runner.
"""
...
RULE_IDS = CONFIG["rules"]
OUT_DIR = HERE / CONFIG["enumeration"]["output_dir"]
